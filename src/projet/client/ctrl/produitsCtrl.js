...
function chargerProduitsSuccess(data, text, jqXHR) {
  ...
  if (sessionStorage.getItem("isAdmin") === "1") {
    $(data).find("produit").each(function () {
      ...
      contenu+= `<input type="text" id="textInput" value ="${$(this).find("nomP").text()}" placeholder="nom du produit" disabled>`;
      contenu += `<label for="inputId"><strong>Description :</strong></label>`;
      contenu+= `<input type="text" id="textInput" value ="${$(this).find("description").text()}" placeholder="description du produit" disabled>`;
      contenu += `<label for="inputId"><strong>Lien image :</strong></label>`;
      contenu+= `<input type="text" id="textInput" value ="${$(this).find("lienImage").text()}" placeholder="lien de l'image du produit" disabled>`;
      contenu += `<label for="inputId"><strong>Prix :</strong></label>`;
      contenu+= `<input type="text" id="number" value ="${$(this).find("prix").text()}" placeholder="prix du produit" disabled>`;
      contenu += `<label for="inputId"><strong>Id categorie :</strong></label>`;
      contenu+=`<input type="text" id="number" value ="${$(this).find("categorie").find("id").text()}" placeholder="id de la categorie" disabled>`
      contenu += `<label for="inputId"><strong>Id marque :</strong></label>`;
      contenu+=`<input type="text" id="number" value ="${$(this).find("marque").find("id").text()}" placeholder="id de la marque" disabled>`
      contenu += `</div>`;
      contenu += `<div class="card-body">`;
    ...
    document.getElementById("btnAjoutPlace").innerHTML = `<button id="btnAjouter">Ajouter</button>`;
...
$(document).on("click", ".btn-add", function () {
  ...
  if (!allFilled) {
    ...
    addProduit(nom, description, lien_Image, prix, FK_Categorie, FK_Marque,
        function () {
